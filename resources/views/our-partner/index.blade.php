...
@section('content')
            ...
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0 card-title d-flex align-items-center justify-content-between">{{ $title }}
                        ...
                        <button type="button" onclick="showFormModal('New Partner','Save')" class="btn btn-sm btn-primary rounded-1">Add Our Partner</button>
                        @endif
                    </h4>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table-sm table-striped table-bordered table table-hover mb-0" id="blog_table">
                            <thead>
                                ...
                                <th>Created By</th>
                                <th>Created At</th>
                                @if(permission('our-partner-delete') || permission('our-partner-edit'))
    ...
    @if(permission('our-partner-delete') || permission('our-partner-edit'))
    @include('our-partner.store_or_update')
    @endif
@endsection
...
<script>
    table = $('#blog_table').DataTable({
        ...
        ajax: {
            url: "{{ route('app.our-partners.index') }}",
            type: "GET",
            dataType: "JSON",
            data: function(d) {
                d._token = _token;
        ...
        columns: [
            ...
            {data: 'created_by'},
            {data: 'created_at'},
            @if(permission('our-partner-delete') || permission('our-partner-view') || permission('our-partner-edit'))
            {data: 'action'}
            @endif
        ],
        language: {
    ...
    $(document).on('click','.edit_data',function(){
        ...
        if (id) {
            $.ajax({
                url: "{{ route('app.our-partners.edit') }}",
                type: "POST",
    ...
    $(document).on('click', '.delete_data', function () {
        ...
        let url  = "{{ route('app.our-partners.delete') }}";
    ...
    function multi_delete(){
        ...
        if(ids.length == 0){
            ...
            let url = "{{ route('app.our-partners.bulk-delete') }}";
    ...
    $(document).on('click','.change_status', function(){
        ...
        var url = "{{ route('app.our-partners.status-change') }}"
