            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" id="user_form">
                        ...
                        <x-form.inputbox labelName="Full Name" name="name" required="required" placeholder="Enter full name"/>
                        <x-form.inputbox type="email" labelName="Email" name="email" required="required" placeholder="Enter email"/>
                        <x-form.inputbox labelName="Mobile No" name="mobile_no" required="required" placeholder="Enter mobile number"/>
                        ...
                        <div class="permission-box">
                            <ul id="permission">
                                ...
                                <li>
                                    <input type="checkbox" class="module-all-{{ $module->id }}" onclick="module_check({{ $module->id }})" id="data-{{ $module->id }}" />
                                    <label for="data-{{ $module->id }}">{{ $module->name }}</label>
                                    <ul>
                                        @foreach ($module->permissions as $permission)
                                            <li>
                                                <input type="checkbox" name="permission[]" id="permission-{{ $permission->id }}" class="permission-select-{{ $module->id }}" value="{{ $permission->id }}"> <label for="permission-{{ $permission->id }}">{{ $permission->name }}</label>
                                            </li>
                                        @endforeach
                                    </ul>
                                </li>
...
<script>
    ...
    function module_check(module_id) {
        if ($('.module-all-' + module_id).is(':checked')) {
            $('.permission-select-' + module_id).prop('checked', true);
        } else {
            $('.permission-select-' + module_id).prop('checked', false)
        }
    }
