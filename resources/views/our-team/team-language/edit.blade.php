            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" id="form_data">
                        ...
                        <x-form.inputbox labelName="Name" name="name" value="{{ $edit->name ?? '' }}" required="required" placeholder="Enter Name"/>
...
<script>
    ...
    $(document).on('click','#save-btn',function(){
        ...
        $.ajax({
            type: "POST",
            url: "{{ route('app.team-languages.store') }}",
            data: formData,
            dataType: "JSON",
            contentType: false,
            processData: false,
            ...
            success: function (response) {
                ...
                if (response.status == false) {
                    ...
                    if (response.status == 'success') {
                        setInterval(() => {
                            // window.location.href = "{{ route('app.team-languages.index') }}";
                        }, 1000);
                    }
                }
            }
