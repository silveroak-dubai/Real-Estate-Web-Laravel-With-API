            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" id="blog_form">
                        ...
                        <x-form.inputbox labelName="Title" name="title" value="{{ $edit->title }}" required="required" placeholder="Enter title"/>
                        <x-form.inputbox labelName="Slug" name="slug" value="{{ $edit->slug }}" required="required" placeholder="Enter slug"/>
                        ...
                        <x-form.selectbox labelName="Is Comment" name="is_comment" required="required">
                            ...
                            <option value="{{ $key }}" {{ $key = $edit->is_comment ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </x-form.selectbox>
                        <x-form.selectbox labelName="Status" name="status" required="required">
                            @foreach (STATUS as $key=>$value)
                            <option value="{{ $key }}" {{ $key = $edit->status ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </x-form.selectbox>
                        <x-form.textarea labelName="Description" value="{{ $edit->description }}" name="description" required="required" placeholder="Enter Description"></x-form.textarea>
                        <x-form.inputbox labelName="Published Date" value="{{ $edit->published_date }}" name="published_date" required="required" placeholder="YYYY-MM-DD"/>
                        <x-form.inputbox type="file" labelName="Image" name="image" required="required"/>
                        <input type="hidden" name="old_image" value="{{ $edit->image }}">
                        <x-form.inputbox labelName="Meta Title" name="meta_title" value="{{ $edit->meta_title }}" placeholder="Enter title"/>
                        <x-form.textarea labelName="Meta Description" name="meta_description" value="{{ $edit->meta_description }}" placeholder="Enter description"></x-form.textarea>
                    </form>
